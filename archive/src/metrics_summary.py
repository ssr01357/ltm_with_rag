...
def m01_performance_report(df, ans_cols): #ans_cols : a list of length 3
    boot_rs_lst = []
    for i, ans_col in enumerate(ans_cols):
        print(f"ID {i}: {ans_col}")
        df[f'{i}_coded_pred'] = df[ans_col].str.contains('Yes', case=False)
        df[f'{i}_Has_Yes_No'] = df[ans_col].str.contains('Yes|No', case=False)
        effective_index = df[f"{i}_Has_Yes_No"] == True
        coded_pred = df[effective_index][f'{i}_coded_pred'].to_list()
        ...
        print()
        boot_rs_lst.append(_boostrap_resampling_report(m_labels, coded_pred))
        print()
        print()

    return boot_rs_lst, df
    
def t14_performance_report(df, ans_cols):
    boot_rs_lst = []
    for i, ans_col in enumerate(ans_cols):
        print(f"ID {i}: {ans_col}")
        df[f'{i}_Has_Valid_Prediction_t'] = df[ans_col].str.contains('T1|T2|T3|T4', case=False)
        ...
        for _, row in df.iterrows():
            if "T1" in row[ans_col]:
                ...
                coded_pred_list.append(-1)
        df[f'{i}_coded_pred_t'] = coded_pred_list

        effective_index = df[f"{i}_Has_Valid_Prediction_t"] == True
        coded_pred = df[effective_index][f'{i}_coded_pred_t'].to_list()
        ...
        print()
        boot_rs_lst.append(_boostrap_resampling_report(t_labels, coded_pred))
        print()
        print()

    return boot_rs_lst, df

def n03_performance_report(df, ans_cols):
    boot_rs_lst = []
    # check if the ans_col contain any valid prediction (e.g., T1, T2, T3, T4)
    for i, ans_col in enumerate(ans_cols):
        print(f"ID {i}: {ans_col}")
        # df[f'{i}_Has_Valid_Prediction_n'] = df[ans_col].str.contains('N0|N1|N2|N3', case=False)
        df[f'{i}_Has_Valid_Prediction_n'] = df[ans_col].str.contains('N0|N1|N2|N3', case=False) & (df['n'] != -1)
        # transform the prediction string to code
        coded_pred_list = []
        for _, row in df.iterrows():
            ...
            if "N0" in row[ans_col]:
                ...
                # Has_Valid_Prediction == False
                coded_pred_list.append(-1)
        df[f'{i}_coded_pred_n'] = coded_pred_list

        effective_index = df[f"{i}_Has_Valid_Prediction_n"] == True
        coded_pred = df[effective_index][f'{i}_coded_pred_n'].to_list()
        ...
        print()
        boot_rs_lst.append(_boostrap_resampling_report(n_labels, coded_pred))
        print()
        print()
    return boot_rs_lst, df